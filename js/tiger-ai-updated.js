/* ==========================================================
   Tiger AI Assistant — UPDATED VERSION (Product-Aware + Tracking)
   ----------------------------------------------------------
   التحسينات الرئيسية:
   1. يجلب بيانات المنتجات الفعلية من Firebase
   2. يضيف روابط حقيقية قابلة للنقر لكل منتج
   3. يدعم تتبع الطلبات مع روابط Bosta المباشرة
   4. System Prompt محسّن لدقة المعلومات
   ========================================================== */
...
(function () {
  ...
  // ========= Config =========
  const TIGER_AI_VERSION = '2.0.0-product-aware';

  // Brand-based assistant icon
  ...
  // Public path: readable by all clients
  const TIGER_AI_PUBLIC_PATH = 'aiConfig/public';
  // Secret path: only readable when authenticated
  const TIGER_AI_SECRET_PATH = 'aiConfig/secret';
  // Default NVIDIA API endpoint
   ...
   * Returns array of product objects with id, name, price, category, colors, sizes
   */
  async function loadProducts(force = false) {
    ...
    try {
      ...
      console.log(`[TigerAI] Loaded ${allProductsSnapshot.length} products`);
  ...
  /**
   * Format product data for AI context with REAL clickable links
   * This is the KEY function that makes products findable with real URLs
   */
  function formatProductForAI(product) {
    ...
    const sizes = Array.isArray(product.sizes) 
      ? product.sizes.join(', ')
      : (product.sizes ? Object.keys(product.sizes).join(', ') : '');
    
    // Format colors with names
    let colorsStr = '';
    if (Array.isArray(product.colors)) {
      colorsStr = product.colors.map(c => typeof c === 'string' ? c : c.name).join(', ');
    } else if (product.colors && typeof product.colors === 'object') {
      colorsStr = Object.values(product.colors).map(c => typeof c === 'string' ? c : c.name).join(', ');
    }
    
    // Generate REAL product link - this is the main fix!
    const productLink = `${SITE_URL}/product?id=${productId}`;
   ...
   💰 السعر: ${fmtPrice(price)}
   🏷️ الفئة: ${category}
   🎨 الألوان المتاحة: ${colorsStr || 'غير متوفر'}
   📏 المقاسات المتاحة: ${sizes || 'غير متوفر'}
   🔗 الرابط: [عرض المنتج](${productLink})`
    };
  }

  /**
   * Build products context string for AI system prompt
   * This injects REAL product data into every conversation
   */
  async function buildProductsContext() {
    ...
    let context = '\n## 📦 قائمة المنتجات الحالية في المتجر (مع روابط حقيقية):\n';
    context += '---\n';
    ...
    context += '\n---\n';
    context += '**مهم جداً:** عندما يطلب العميل منتجاً، أرشده للمنتج المناسب وأعطه الرابط أعلاه مباشرة.\n';
    context += 'استخدم دائماً التنسيق: [اسم المنتج](رابط المنتج) للروابط القابلة للنقر.\n';
  ...
  async function callNvidiaAPI(messages, options = {}) {
    ...
    const body = {
      ...
      messages: messages,
      max_tokens: options.maxTokens || aiConfig?.maxTokens || 2048, // Increased for product data
      temperature: options.temperature ?? aiConfig?.temperature ?? 0.6, // Slightly lower for accuracy
      stream: false
    };
  ...
  /**
   * Build enhanced system prompt with real product data
   * This is the MAIN improvement - AI now has access to actual products
   */
  async function buildSystemPromptAsync() {
    ...
    if (isAdminMode) {
      return `${aiConfig?.adminSystemPrompt || `أنت "تايجر AI" — المساعد الإداري لمتجر Tiger Jeans. لديك صلاحية كاملة لرؤية كل البيانات.
...
- إذا سئلت عن منتج غير موجود، اقترح بدائل مشابهة من المنتجات المتاحة`;
    }

    // ====== CUSTOMER MODE (Enhanced) ======
    return `${aiConfig?.systemPrompt || `أنت "تايجر AI" المساعد الذكي الرسمي لمتجر Tiger Jeans (تايجر جينز).

## مهمتك الأساسية:
مساعدة العملاء في العثور على المنتجات المناسبة والإجابة على استفساراتهم بناءً على البيانات الحالية للمتجر فقط.

## قواعد صارمة (لا تكسرها أبداً):
1. **أجب فقط من المنتجات المذكورة أدناه** - لا تخترع منتجات وهمية
2. **كل رابط منتج يجب أن يكون حقيقي وقابل للنقر** بالتنسيق: [اسم المنتج](رابط كامل)
3. **اذكر دائماً**: اسم المنتج، السعر، المقاسات المتاحة، الألوان، ورابط المنتج الحقيقي
4. **لا تذكر أبداً** تكلفة المنتج الأصلية (costPrice) أو بيانات العملاء الآخرين
5. استخدم لهجة عربية مصرية واضحة وودودة

${productsContext}

## معلومات المتجر:
- الاسم: Tiger Jeans (تايجر جينز)
- التخصص: بناطيل جينز وملابس عصرية فاخرة
- الشحن: لجميع محافظات مصر عبر شركة Bosta (بوستا)
- الدفع: كاش عند الاستلام، فودافون كاش، إنستا باي، بطاقات هدايا
- سياسة الاسترجاع: يمكن استرجاع/استبدال المنتج خلال 7 أيام
...
## تتبع الطلبات:
- إذا طلب العميل تتبع طلبه، اطلب منه رقم هاتفه أو رقم الطلب
- عند وجود رقم تتبع، قدم رابط التتبع المباشر من شركة Bosta:
  - رابط Bosta: https://app.bosta.co/track/{trackingNumber}

## نبرة الصوت:
- ودودة ومهذبة مثل صديق خبير
...
- كن مباشراً ومفيداً`;
  ...
  function buildSystemPrompt() {
    // Return a basic prompt synchronously - will be enhanced async later
    if (isAdminMode) {
      return aiConfig?.adminSystemPrompt || 'أنت مساعد ذكي لمتجر Tiger Jeans (وضع الأدمن).';
    }
    return aiConfig?.systemPrompt || 'أنت "تايجر AI" المساعد الذكي لمتجر Tiger Jeans.';
  }

  // ========= Messages Context Builder (ENHANCED) ==========
  
  /**
   * Build messages context with real product data
   * Now loads products and includes them in context
   */
  async function buildMessagesContextAsync(userMessage) {
    // Build enhanced system prompt with products
    ...
    const trimmedHistory = chatHistory.slice(-6); // Keep last 6 messages for context

    return {
      messages: [systemMsg, ...trimmedHistory, userMessage],
      context: { hasProducts: productsCache && productsCache.length > 0 }
    };
  }
    ...
    root.innerHTML = `
      ...
      <div class="tj-ai-panel" id="tjAiPanel" role="dialog" aria-label="Tiger AI Assistant">
        ...
        <div class="tj-ai-quick" id="tjAiQuick">
          ...
          <button class="tj-ai-quick-chip" data-prompt="عاوز أتبع طلبي - هل ممكن؟"><i class='bx bx-package'></i> تتبع الطلبات</button>
        </div>
   ...
   * ENHANCED: Now properly renders product links as clickable
   */
  function formatMessage(text) {
    let html = escapeHtml(text);
    
    // Links: [نص](https://...) -> real clickable <a>
  ...
  // ========= Send Message (ENHANCED with Product Context) ==========
  async function sendMessage() {
    ...
    try {
      // ENHANCED: Use async context builder with real products
      const { messages, context } = await buildMessagesContextAsync({ 
        role: 'user', 
        content: finalText 
      });
...
لتنفيذه، اضغط زر "**نفذ**" ثم أرسل نفس الرسالة مرة أخرى.`);
        ...
        // Customer mode - simple call with enhanced context
  ...
  // ========= Track Orders Feature (ENHANCED with Real Tracking Links) =========
...
لم نتمكن من العثور على رقم هاتف مربط بحسابك.
...
يرجى إدخال رقم مصري صحيح (يبدأ بـ 01)`);
  ...
  /**
   * Search and display orders with REAL tracking links
   * ENHANCED: Now provides direct Bosta tracking links
   */
...
شكراً لتواصلك مع متجر تايجر جينز! 🐯`);
      ...
      // Build orders display with REAL tracking links
      ...
      customerOrders.forEach((order, idx) => {
        ...
        const shippingCompany = order.shippingCompany || order.shipping?.company || '';
        const trackingNumber = order.trackingNumber || order.shipping?.trackingNumber || '';
        
        message += `---\n**طلب #${idx + 1}** (${order.code || order.orderCode || order.id})\n`;
        ...
        if (order.items && order.items.length > 0) {
          ...
          order.items.forEach((item, i) => {
            message += (i > 0 ? '، ' : '') + item.name;
          });
          message += `\n`;
        }
        
        if (shippingCompany) {
          const companyName = getShippingCompanyName(shippingCompany);
          message += `🚚 شركة الشحن: ${companyName}\n`;
        }
        
        // ENHANCED: Real tracking link for Bosta
        if (trackingNumber) {
          const trackingLink = generateTrackingLink(shippingCompany, trackingNumber);
          message += `📦 رقم التتبع: ${trackingNumber}\n`;
          if (trackingLink) {
            message += `🔗 [تتبع الشحنة مباشرة](${trackingLink})\n`;
          }
        }
...
حاول مرة أخرى لاحقاً.`);
  ...
  /**
   * Get shipping company name in Arabic
   */
  function getShippingCompanyName(company) {
    if (!company) return 'جاري التحديد';
    ...
    for (const [key, name] of Object.entries(names)) {
      if (companyLower.includes(key)) return name;
    }
    return company;
  }

  /**
   * Generate REAL tracking link based on shipping company
   * ENHANCED: Direct links that work!
   */
  function generateTrackingLink(company, trackingNumber) {
    ...
    // Bosta (Main shipping partner in Egypt)
    if (companyLower.includes('bosta') || companyLower.includes('بوستا')) {
      return `https://app.bosta.co/shipments/${trackingNumber}`;
    }
  ...
  async function init() {
    ...
    try {
      ...
      // Auto-load Firebase + config.js if missing
...
تعذر الاتصال بخادم البيانات. تحقق من اتصال الإنترنت وحاول مرة أخرى.`, true);
  ...
  async function continueInit() {
    ...
    // Pre-load products for faster responses
    console.log('[TigerAI] Pre-loading products...');
    await loadProducts();
    console.log('[TigerAI] Products loaded and cached');

    // Hide launcher if disabled and not admin
  ...
  window.TigerAI = {
    ...
    testNvidiaConnection: async function(testApiKey) {
      const url = `${getNvidiaApiUrl()}/chat/completions`;
    ...
    debug: function() {
      return {
        ...
        isLoading,
        productsCached: productsCache ? productsCache.length : 0
      };
    },
    ...
    // NEW: Force reload products cache
