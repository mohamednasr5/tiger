...
// ---------- FIX/SYNC MISSING CARDS ----------
// This function finds approved requests that don't have corresponding entries in giftCards
async function fixMissingGiftCards() {
  if (!confirm("⚡ هل تريد إصلاح البطاقات الناقصة؟\n\nسيتم إنشاء بطاقات لكل الطلبات الموافق عليها التي لم تُنشأ بعد في giftCards.")) return;
  
  try {
    showToast("جارٍ فحص البطاقات... ⏳");
    
    const requestsSnap = await db.ref("giftCardRequests").once("value");
    const requests = requestsSnap.val();
    const cardsSnap = await db.ref("giftCards").once("value");
    const existingCards = cardsSnap.val() || {};
    ...
    if (requests) {
      for (const reqId of Object.keys(requests)) {
        ...
        // Only process approved/activated requests
        if (req.status !== 'approved' && req.status !== 'activated') continue;
        
        // Check if card already exists in giftCards
        const cardKey = (req.cardNumber || '').replace(/-/g, '');
        
        if (!cardKey) {
          console.log(`⚠️ Request ${reqId} has no cardNumber, skipping`);
          continue;
        }
        ...
        // Create missing card
        const pin = genGiftCardPin ? genGiftCardPin() : Math.floor(100000 + Math.random() * 900000).toString();
        const expiresAt = now + (GC_ADMIN_EXPIRY_MS || (365 * 24 * 60 * 60 * 1000));
        ...
        const card = {
          ...
          ownerPhone: req.senderPhone || null,
          recipientName: req.recipientName || null,
          occasion: req.occasion || null,
          message: req.message || null,
          requestId: reqId,
          createdAt: req.createdAt,
          activatedAt: req.approvedAt || now,
          expiresAt: expiresAt,
          timeline: [
            { event: "created", ts: req.createdAt },
            { event: "payment_received", ts: req.createdAt },
            { event: "approved", ts: req.approvedAt || now },
            { event: "activated", ts: req.approvedAt || now },
            { event: "fixed_synced", ts: now }
          ],
          _fixedAt: now,
          _fixedBy: "admin_sync"
        };
        ...
        await db.ref("giftCards/" + cardKey).set(card);
        console.log(`✅ Fixed missing card: ${req.cardNumber} for ${req.recipientName}`);
        fixedCount++;
      }
    }
    
    if (fixedCount > 0) {
      showToast(`✅ تم إصلاح ${fixedCount} بطاقة ناقصة بنجاح!`);
      loadGiftCards(); // Refresh the list
    } else if (skippedCount > 0) {
      showToast(`✅ كل البطاقات سليمة (${skippedCount} موجودة مسبقاً)`);
    } else {
      showToast("لا توجد بطاقات ناقصة للإصلاح");
    }
    
  } catch (e) {
    console.error(e);
    showToast("حدث خطأ أثناء الإصلاح");
  }
}

// Add button to admin UI for fixing missing cards
function addFixMissingButton() {
  const header = document.querySelector('.gc-admin-header');
  if (!header) return;
  
  // Check if button already exists
  if (document.getElementById('fixMissingBtn')) return;
  
  const btn = document.createElement('button');
  btn.id = 'fixMissingBtn';
  btn.className = 'btn-secondary';
  btn.style.cssText = 'margin-right:auto;font-size:.85rem';
  btn.innerHTML = '<i class=\'bx bx-sync\'></i> إصلاح البطاقات الناقصة ⚡';
  btn.onclick = fixMissingGiftCards;
  btn.title = 'إنشاء بطاقات للطلبات الموافق عليها التي لم تُنشأ بعد';
  
  header.appendChild(btn);
}

// Call after page loads
setTimeout(addFixMissingButton, 1000);
