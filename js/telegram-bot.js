...
🔖 <b>رقم الطلب:</b> <code>${order.orderCode || order.code || order.id}</code>
...
<i>يرجى مراجعة الإيصال والموافقة على الطلب</i>
  ...
  // ✅ إرسال فوري عبر Notification Service (يعمل حتى لو المتصفح مقفول)
  try {
    await fetch(`${TELEGRAM_CONFIG.workerUrl}/api/notify/payment`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Bot-Token': TELEGRAM_CONFIG.botToken,
    ...
    console.log('✅ Payment notification sent to service');
  } catch (e) {
    console.log('Service notify failed, using fallback');
  }
